...
class ChatService {
  ...
  async initiateChat(senderId, recipientCodename) {
    const recipient = await userPublicKey.getUserByCodename(recipientCodename);
  ...
  async sendMessage(senderId, recipientCodename, content) {
    ...
    const encryptedContent = crypto.publicEncrypt(sender_public_key, Buffer.from(content)).toString('base64');
  ...
  async getMessages(userId, recipientCodename) {
    ...
    const [session] = await pool.query(
      'SELECT sender_private_key FROM chat_sessions WHERE sender_id = ? AND recipient_id = ?',
      [userId, recipient.id]
    );
    ...
    return messages.map((msg) => ({
      ...msg,
      content: session[0]
        ? crypto.privateDecrypt(session[0].sender_private_key, Buffer.from(msg.content, 'base64')).toString()
        : msg.content,
    }));
  }
}
